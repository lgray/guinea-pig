...
//void distributePhotonInBeam(   vector< list<BEAM_PARTICLE_POINTER> >& grid_photon, BEAM* beam, int slice,float ratio, float ratio_i, RNDM& rndm_generator);
void distributePhotonInBeam(   vector< list<BEAM_PHOTON_POINTER> >& grid_photon, int i_beam, int slice,float ratio, float ratio_i, RNDM& rndm_generator);
...
inline   void saveOnHadronFile(int num, float energy1, float energy2, float min_z, RNDM& rndm_generator )
{
  int i;
  for (i=0;i<num;i++)
    {
...
void move_pairs(const vector<GENERAL_GRID*>& grids, PAIR_BEAM& pairs, int i_slice, double d_eps_1, double d_eps_2, int extra_grids, float charge_sign_0, RNDM& rndm_generator);

void move_pairs_tertphot(const vector<GENERAL_GRID*>& grids, PAIR_BEAM& pairs, int i_slice, double d_eps_1, double d_eps_2, int extra_grids, float charge_sign_0, RNDM& rndm_generator);
