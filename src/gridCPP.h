...
//! for each cell, a list of pointers to particles assigned to this celle (beam1)
  vector< list<BEAM_PARTICLE_POINTER> > part_pointer1_;
//! for each cell, a list of pointers to particles assigned to this celle (beam2)
  vector< list<BEAM_PARTICLE_POINTER> > part_pointer2_;
  
//! for each cell, a list of pointers to photons assigned to this celle (beam1)
  vector< list<BEAM_PHOTON_POINTER> > grid_photon1_;
//! for each cell, a list of pointers to photons assigned to this celle (beam2)
  vector< list<BEAM_PHOTON_POINTER> > grid_photon2_;

//! for each cell, a list of pointers to extra_photons assigned to this celle (beam2)
vector< list<EXTRA_PHOTON_POINTER> > extra_photon_pointer1_;
//! for each cell, a list of pointers to extra_photons assigned to this celle (beam2)
vector< list<EXTRA_PHOTON_POINTER> > extra_photon_pointer2_;

// A vector of the photons generated by incoherent pairs. They do not iteract with the beam.
vector<TERTPHOTON> tertphot_;
...
inline float spread_energy(float ener, int which_spread, float spread, RNDM& hasard) const 
{
  ...
  switch(which_spread)
    ...
    case 0:
      break;
    case 1:
      energy *= 1.0 + spread*(hasard.rndm()*dx+xmin);
      break;
    case 2:
      if ( hasard.rndm() < 0.5 )
	{
      ...
      else
	{
      ...
      break;
    case 3:
      energy *= 1.0 + spread*hasard.gasdev();
      break;
    }
  return energy;
}
...
void collide_ge(int cellx, int celly,float min_z, const BEAM_PHOTON_POINTER& photon_pointer, const BEAM_PARTICLE_POINTER& particle_pointer, PAIR_BEAM& secondaries, SWITCHES& switches, const PAIR_PARAMETER& pair_parameter, RNDM& hasard);

void collide_eg(int cellx, int celly,float min_z,const BEAM_PARTICLE_POINTER& particle_pointer, const BEAM_PHOTON_POINTER& photon_pointer, PAIR_BEAM& secondaries, SWITCHES& switches, const PAIR_PARAMETER& pair_parameter, RNDM& hasard);

void collide_gg(int cellx, int celly, float min_z, const BEAM_PHOTON_POINTER& photon_pointer1, const BEAM_PHOTON_POINTER& photon_pointer2, PAIR_BEAM& secondaries, PAIR_BEAM& muons, SWITCHES& switches, RNDM& hasard, double *returnCrossSectionToAdd);
...
inline void collide_compton(int index_of_process, int cellx, int celly,float min_z, const PHOTON_POINTER& extr_phot, const BEAM_PARTICLE_POINTER& particle,PAIR_BEAM& secondaries,SWITCHES& switches, RNDM& hasard, int dir)
{
  if (switches.get_do_compt())
    {
      ...
      compt_.compt_do(mesh_, cellx, celly,min_z, secondaries,index_of_process,particleEnergy,photonEnergy,photon_q2,particleVx, particleVy,weight,dir,switches, hasard);
    }
}

void collide_gg_XX(int index_of_process, int cellx, int celly,float min_z, const PHOTON_POINTER& photon1, const PHOTON_POINTER& photon2, SWITCHES& switches, PAIR_BEAM& secondaries, PAIR_BEAM& muons, RNDM& hasard, double *returnCrossSectionToAdd);

void make_hadrons_gg2(float min_z, float energy1,float q2_1,float energy2,float q2_2,float lumi, SWITCHES& switches,double *returnCrossSectionToAdd, RNDM& hasard);

void isr2(float e1,float e2,float *e1p,float *e2p, RNDM& hasard);
...
int coherent_generate(BEAM& beam,int i_slice, float upsilon,PHOTON& phot, RNDM& hasard);

bool pick_coherent_energy(float ups,float& energy, RNDM& hasard);

void apply_electric_field_on_pair(float step_2, float ex, float ey, float& vx,float& vy,float e_inv) const
{
  // application du champ electrique sur un pas : step_2
...
//void step_pair_1X(const vector<GENERAL_GRID*>& grids, PAIR_PARTICLE& paire,float step,int n, int extra_grids, float charge_sign_0, RNDM& hasard);
void step_pair_1(const vector<GENERAL_GRID*>& grids, PAIR_PARTICLE& paire,double mass,float step,int n, int extra_grids, float charge_sign_0, RNDM& hasard);
void step_pair_1_tertphot(const vector<GENERAL_GRID*>& grids, PAIR_PARTICLE& paire,double mass,float step,int n, int extra_grids, float charge_sign_0, RNDM& hasard);
...
//void distributePhotonInBeam(   vector< list<BEAM_PARTICLE_POINTER> >& grid_photon, BEAM* beam, int slice,float ratio, float ratio_i, RNDM& hasard);
void distributePhotonInBeam(   vector< list<BEAM_PHOTON_POINTER> >& grid_photon, int i_beam, int slice,float ratio, float ratio_i, RNDM& hasard);
...
//void distributeCoherentScatterInBeam1(int i_beam, int i_slice, float ratio, float ratio_i, vector< list<BEAM_PARTICLE_POINTER> >& part_pointer);
// void distributeCoherentScatterInBeam2(int i_beam, int i_slice, float ratio, float ratio_i, vector< list<BEAM_PARTICLE_POINTER> >& part_pointer);
...
void move_particles(const vector<GENERAL_GRID*>& grids, int i_beam, int i_slice, int interpolation,int do_beamstrahlung,int do_trident, int sokolov, float emin, int do_prod, int extra_grids, float charge_sign,  int bmt_rotate);

void move_coherent_particles(const vector<GENERAL_GRID*>& grids, int i_beam, int i_slice, int interpolation, int do_beamstrahlung,float emin, int do_prod,int extra_grids, float charge_sign);

void move_trident_particles(const vector<GENERAL_GRID*>& grids, int i_beam, int i_slice, int interpolation, int do_beamstrahlung,float emin, int do_prod,int extra_grids, float charge_sign);

inline TRIVECTOR EBfieldOnParticle(const PARTICLE* particle, const vector<GENERAL_GRID*>& grids, const PHI_FLOAT *phi, int i_beam, int extra_grids) const
{
  float xpart, ypart;
  TRIVECTOR EBfield;
...
//void mequiv (double s4, double lns4, float xmin,float e,int iflag,float *eph,float *q2,float *one_m_x, RNDM& hasard);

inline   void saveOnHadronFile(int num, float energy1, float energy2, float min_z, RNDM& hasard )
{
  int i;
  for (i=0;i<num;i++)
    {
      hadron_file_->save_hadron(energy1,energy2,(1e-3*(min_z+hasard.rndm())*mesh_.get_delta_z()));
    }
}
...
inline void connect_random_generator( RNDM* hasard) 
{
  hasard_ = hasard;
}
...
inline const RESULTS& get_results() const { return results_;}
...
inline void init_extra_photons()
{
  ...
  //  extra_photons_cell1_ = vector< list<EXTRA> >(totalSize);
  //   extra_photons_cell2_ = vector< list<EXTRA> >(totalSize);
  extra_photon_pointer1_ = vector< list<EXTRA_PHOTON_POINTER> >(totalSize);
  extra_photon_pointer2_ = vector< list<EXTRA_PHOTON_POINTER> >(totalSize);
  clear_extra_photons();
...
void distribute_photons(int slice_1,int slice_2, float photon_ratio, RNDM& hasard);
...
inline void moveAllParticles(const vector<GENERAL_GRID*>& grids,  int i_slice1, int i_slice2 , int interpolation,int do_beamstrahlung, int sokolov, float emin, int do_prod, int extra_grids, float charge_sign,int bmt_rotate,int do_trident)
{
...
void move_photons2(BEAM& beam,int ibeam,int i_slice,RNDM& hasard);

void move_pairs(const vector<GENERAL_GRID*>& grids, PAIR_BEAM& pairs, int i_slice, double d_eps_1, double d_eps_2, int extra_grids, float charge_sign_0, RNDM& hasard);

void move_pairs_tertphot(const vector<GENERAL_GRID*>& grids, PAIR_BEAM& pairs, int i_slice, double d_eps_1, double d_eps_2, int extra_grids, float charge_sign_0, RNDM& hasard);

void photon_lumi(float min_z,SWITCHES& switches, PAIR_BEAM& secondaries, PAIR_BEAM& muons, RNDM& hasard);

void photon_lumi_2(float min_z,SWITCHES& switches,PAIR_BEAM& secondaries, PAIR_BEAM& muons, RNDM& hasard);

void photon_lumi_3(float min_z,SWITCHES& switches, PAIR_BEAM& secondaries,RNDM& hasard);
